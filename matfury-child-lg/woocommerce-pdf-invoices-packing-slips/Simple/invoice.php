...
<table class="order-details">
	<thead>
		<tr>
			...
			<!-- ADD NEW HERE (Haziq - 03/04/2022) - add uom column in invoice -->
			<th class="uom"><?php _e( 'UOM', 'woocommerce-pdf-invoices-packing-slips' ); ?></th>
	...
	<tbody>
		<?php foreach ( $this->get_order_items() as $item_id => $item ) : ?>
			<tr class="<?php echo apply_filters( 'wpo_wcpdf_item_row_class', 'item-'.$item_id, $this->get_type(), $this->order, $item_id ); ?>">
				...
				<!-- ADD NEW HERE (Haziq - 03/04/2022) - display uom in invoice -->
				<?php
					$woo_uom_output = get_post_meta( $item['product_id'], '_woo_uom_input', true );

					// show UOM if it exists.
					if ( $woo_uom_output ) {
						?><td class="uom"><?php echo esc_attr( $woo_uom_output ); ?></td><?php
					}

					// If UOM not set, show the default text.
					else {
						?><td class="uom"> - </td><?php
					}
				?>
	...
	<tfoot>
		<tr class="no-borders">
			...
			<td></td> <?php // ADD NEW HERE - Haziq (03/02/2022) ?>
			<?php // default colspan="2". Changed to colspan="3". Haziq (03/02/2022) ?>
